...
凡引擎推送给 UI 的快照对象，建议在适配层统一带上（字段名供 JSON / TS 类型对齐用；若落地 JSON，字段名用**中文**与项目主规范一致）：
...
TypeScript 中接口名用英文；若序列化为 JSON，**属性键名用中文**（与主规范一致）。
...
与 [角色系统.md](../01-系统规范/角色系统.md) 对齐的**展示用子集**，不含秘密策划用字段时可裁剪。
